...
func messagesRouter(conf *config.Config, client http.Client) chi.Router {
	...
	router.Post("/", apiHandler(func(logger *slog.Logger, w http.ResponseWriter, r *http.Request) (any, error) {
	...
	router.Delete("/{message_id}", apiHandler(func(logger *slog.Logger, w http.ResponseWriter, r *http.Request) (any, error) {
	...
	router.Put("/{message_id}/reactions/{emoji_id}/@me", apiHandler(func(logger *slog.Logger, w http.ResponseWriter, r *http.Request) (any, error) {
		_, err := performFluxerRequest(w, r, client, &http.Request{
			Method: "PUT",
			URL:    formatFluxerURL(conf, "/channels/%s/messages/%s/reactions/%s/@me", r.PathValue("channel_id"), r.PathValue("message_id"), r.PathValue("emoji_id")),
		})
		if err != nil {
			return nil, fmt.Errorf("failed to perform fluxer request: %w", err)
		}
